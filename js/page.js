(function () {
  ...
  const {filterSongs,paginate,pageWindow,randomSong,formatOrder} = window.Songbook;
  ...
  function render(){
    ...
    pages.innerHTML=pageWindow(paged.page,paged.pages).map(value=>value==="…"?'<span class="page-gap" aria-hidden="true">…</span>':`<button class="page ${value===paged.page?'active':''}" data-page="${value}" aria-label="第 ${value} 页">${value}</button>`).join("");
