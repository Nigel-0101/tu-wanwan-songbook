(function () {
  ...
  const body = $("#songRows"), count = $("#songCount"), notice = $("#notice"), pages = $("#pages");
  ...
  function announce(text){ notice.textContent=text; notice.classList.remove("pop"); void notice.offsetWidth; notice.classList.add("pop"); }
  ...
  body.addEventListener("click",async e=>{const b=e.target.closest(".copy");if(!b)return;const song=songs.find(item=>item.id===b.dataset.songId);if(!song)return;const order=formatOrder(song);try{await navigator.clipboard.writeText(order);showSelectionCapsule(order);}catch{announce(`请手动复制：${order}`);}b.textContent="已复制";setTimeout(()=>b.textContent="复制歌名",900);});
  $("#random").addEventListener("click",()=>{const song=randomSong(filtered());if(!song)return announce("当前筛选下没有可选歌曲");const order=formatOrder(song);showSelectionCapsule(order);announce(order);});
